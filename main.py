...
@bot.event
async def on_message(message):   # when user sends a message
    if not message.author == bot.user:
        print('Message from {0.author}: {0.content}'.format(message))
    await bot.process_commands(message)   # logs what the user sends

# make the bot send to chat when someone requests info for a pokemon
@bot.command()    # use 2 to declare event or command
async def info(ctx, arg, *options):    # ctx: context arg: pokemon name
    ...
    # Step 2: take our data and parse into vars for future use -> make it easier to get this info later on, so put in variables
    ...
    # add options abilities, types, and/or shiny options
